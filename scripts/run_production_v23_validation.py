...
def build_workflow(template, avatar_img_name, prev_scene_img_name, prompt, negative, seed, weight=0.15, end_at=0.30, slot2_active=True):
    ...
    # Slot 2: Context-Aware Dynamic Continuity Policy
    if slot2_active and prev_scene_img_name:
...
def evaluate_character(char, template, evaluator):
    ...
    for step in char["turns"]:
        ...
        # PR #23 Context-Aware Dynamic Slot 2 Resolution
        is_cold_start = (turn == 1)
        if is_cold_start:
            weight, end_at, slot2_active = 0.0, 0.0, False
        elif is_transition:
            weight, end_at, slot2_active = 0.08, 0.20, True
        else:
