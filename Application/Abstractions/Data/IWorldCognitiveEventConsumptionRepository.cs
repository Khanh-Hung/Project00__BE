...
/// Enforces character isolation, deterministic retrieval, and database-level idempotency protection.
/// </summary>
public interface IWorldCognitiveEventConsumptionRepository
{
    ...
    /// Returns (true, consumption) if newly claimed as winner.
    /// Returns (false, existing) if already claimed or consumed with matching fingerprint.
    ...
    /// <summary>
    /// Transitions consumption state to Failed with failure explanation.
    /// </summary>
    Task MarkFailedAsync(Guid eventId, string failureReason, CancellationToken ct = default);
